            ...
            return (
                <React.Fragment key={label}>
                    <div className="flex flex-col items-center gap-1">
                        <div
                            className={`flex items-center justify-center w-7 h-7 rounded-full border-2 transition-all duration-300 ${done
                                    ? "bg-[#63C1BB] border-[#63C1BB]"
                                    : active
                                        ? "bg-white border-[#63C1BB]"
                        ...
                        <span className={`text-[10px] font-semibold tracking-wide ${active ? "text-white" : "text-white/50"}`}>
                            {label}
                        </span>
                    </div>
                    {i < steps.length - 1 && (
                        <div
                            className={`flex-1 h-[2px] mx-2 mb-5 rounded-full transition-all duration-300 ${i < current ? "bg-[#63C1BB]" : "bg-white/20"
                                }`}
                            style={{ minWidth: 32 }}
                        />
                    )}
...
const SignupPage = () => {
    ...
    const handleSuccess = (user) => {
        // Auto-login: set session flags in localStorage
