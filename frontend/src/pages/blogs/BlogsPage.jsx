...
import { ArrowLeft, ArrowRight, CalendarDays, SearchX, UserRound } from "lucide-react";
...
const BlogCard = ({ blog }) => (
  ...
  >
    {blog.coverImageUrl ? (
      <img src={blog.coverImageUrl} alt={blog.title} className="h-56 w-full object-cover" />
    ) : (
    ...
    <div className="p-6">
      ...
      <h2 className="text-2xl font-bold leading-tight text-slate-900">{blog.title}</h2>
      <p className="mt-3 line-clamp-3 text-sm leading-7 text-slate-600">{blog.excerpt}</p>
      <Link
        to={`/blogs/${blog.slug}`}
...
const BlogsPage = () => {
  ...
  React.useEffect(() => {
    ...
    const loadBlogs = async () => {
      try {
        setLoading(true);
        setError("");
        const endpoint = slug ? `${API_BASE}/blogs/${slug}` : `${API_BASE}/blogs`;
  ...
  React.useEffect(() => {
    ...
    const pageTitle = activeBlog?.metaTitle || activeBlog?.title || "Blogs | Index Money";
    const description =
    ...
    document.title = pageTitle.includes("Index Money") ? pageTitle : `${pageTitle} | Index Money`;
    setMetaTag("description", description);
    setMetaTag("og:title", document.title, "property");
    setMetaTag("og:description", description, "property");
    setMetaTag("og:type", activeBlog ? "article" : "website", "property");
    if (activeBlog?.coverImageUrl) setMetaTag("og:image", activeBlog.coverImageUrl, "property");
    setCanonical(activeBlog ? `${siteUrl}/blogs/${activeBlog.slug}` : `${siteUrl}/blogs`);
    ...
    if (activeBlog) {
      ...
      script.textContent = JSON.stringify({
        ...
        author: { "@type": "Organization", name: activeBlog.authorName || "Index Money" },
    ...
    return (
      <main className="bg-[#F7FBFA] px-5 py-12 md:px-8 md:py-16">
        <div className="mx-auto max-w-4xl">
          <Link to="/blogs" className="mb-8 inline-flex items-center gap-2 text-sm font-bold text-[#105F68]">
          ...
          {loading ? (
            ...
            <article>
              <header className="mb-8">
                ...
                <p className="mt-5 text-lg leading-8 text-slate-600">{activeBlog.excerpt}</p>
              </header>
    ...
    <main className="bg-[#F7FBFA] px-5 py-12 md:px-8 md:py-16">
      <section className="mx-auto max-w-5xl text-center">
        ...
        <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-slate-600">
          Read published articles from Index Money on market awareness, trading discipline,
          unlisted shares, and long-term investing decisions.
        </p>
      </section>
      ...
      <section className="mx-auto mt-10 max-w-6xl">
        {loading ? (
          ...
          <div className="rounded-[28px] border border-[#D7ECE7] bg-white p-10 text-center">
            ...
            <h2 className="mt-4 text-xl font-bold text-slate-800">No blogs published yet</h2>
